...
class Battery {
    ...
    public Battery(int id, int amountOfColumns, int amountOfFloors, int amountOfBasements, int amountOfElevators, BatteryStatus batteryStatus) {
        this.id = id;
    ...
    public void createColumnsList() {
        ...
        for (int i = 1; i <= this.amountOfColumns; i++) {
            this.columnsList.add(new Column(i, name, ColumnStatus.ACTIVE, this.amountOfElevators, amountOfFloorsPerColumn, amountOfBasements, this));
            name += 1;
        }
    }
    ...
    public int calculateamountOfFloorsPerColumn() {
        ...
        if (this.amountOfBasements > 0) { //if there is basement floors
            amountOfFloorsPerColumn = (this.numberOfFloors / (this.amountOfColumns - 1)); //the first column serves the basement floors
    ...
    public void setColumnValues() {
        ...
        if (this.amountOfColumns == 1) { //if there is just one column, it serves all the floors of the building
            ...
            if (remainingFloors != 0) { //if the remainingFloors is not zero, then it adds the remaining floors to the last column
                this.columnsList.get(this.columnsList.size() - 1).servedFloors = amountOfFloorsPerColumn + remainingFloors;
                this.columnsList.get(this.columnsList.size() - 1).maxFloor = this.columnsList.get(this.columnsList.size() - 1).minFloor + this.columnsList.get(this.columnsList.size() - 1).servedFloors;
            }
    ...
    private void initializeMultiColumnFloors() {
        ...
        for (int i = 1; i < this.columnsList.size(); i++) { //if its not the first column (because the first column serves the basements)
            if (i == 1) {
                ...
                this.columnsList.get(i).servedFloors = (amountOfFloorsPerColumn + 1); //Add 1 floor for the RDC/ground floor
            }
...
class Column {
    ...
    //----------------- Constructor and its attributes -----------------//
    public Column(int id, char name, ColumnStatus columnStatus, int amountOfElevators, int servedFloors, int amountOfBasements, Battery battery) {
        this.id = id;
        this.name = name;
...
class Elevator {
    ...
    //----------------- Constructor and its attributes -----------------//
    public Elevator(int id, int servedFloors, int floor, ElevatorStatus elevatorStatus, SensorStatus weightSensorStatus, SensorStatus obstructionSensorStatus, Column column) {
        this.id = id;
        this.servedFloors = servedFloors;
        this.floor = floor;
...
//            System.out.println("       Elevator" + column.name + this.id + " is now going " + this.status);
    ...
    private void manageButtonStatusOff(int floor) {
        ...
        Optional<Button> currentFloorButton = this.floorRequestButtonsList.stream().filter(button -> button.id == floor).findFirst(); //filter floor button by ID and set status to OFF
